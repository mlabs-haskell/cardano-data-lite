...
import * as thisLib from "./index.js";
...
describe("extendedToPubkey", () => {
    for (let i = 0; i < 10; i++) {
        ...
        test(privateKeyExt.to_hex(), () => {
            expect(naclExt.extendedToPubkey(privateKeyExt.as_bytes())).toEqual(privateKeyExt.to_public().as_bytes());
        });
    }
});
describe("signExtended", () => {
    ...
    test("signature must be same as CSL", () => {
        expect(naclExt.signExtended(msg, privateKeyExt.as_bytes())).toEqual(privateKeyExt.sign(msg).to_bytes());
    });
    test("signature should verify", () => {
        let signature = naclExt.signExtended(msg, privateKeyExt.as_bytes());
        expect(nacl.sign.detached.verify(msg, signature, privateKeyExt.to_public().as_bytes())).toEqual(true);
    });
});
describe("secretToExtended", () => {
    ...
    for (let i = 0; i < 10; i++) {
        let privateKey = regressionKeys[i] != null
            ? CSL.PrivateKey.from_hex(regressionKeys[i])
            : CSL.PrivateKey.generate_ed25519();
        test(privateKey.to_hex(), () => {
            let privateKeyExt = CSL.PrivateKey.from_extended_bytes(thisLib.secretToExtended(privateKey.as_bytes()));
            expect(privateKeyExt.to_public().to_hex()).toEqual(privateKey.to_public().to_hex());
        });
    }
});
describe("bytesToBigIntLE/bigIntToBytesLE", () => {
    ...
    for (let [hex, num] of values) {
        test(hex + ": " + num.toString(), () => {
            const uint8Array = new Uint8Array(Buffer.from(hex, "hex"));
    ...
    for (let i = 0; i < 10; i++) {
        ...
        for (let index of [1, derive.harden(1), 1234, derive.harden(1234)]) {
            test(parentCSL.to_hex() + "/" + index, () => {
                ...
                expect(toHex(thisLib.extendedToPubkey(parent.privateKey))).toEqual(parentCSL.to_public().to_raw_key().to_hex());
                let child = derive.derivePrivate(parent.privateKey, parent.chainCode, index);
    ...
    for (let i = 0; i < 1; i++) {
        ...
        for (let index of [1, 1234]) {
            test(parentCSL.to_hex() + "/" + index, () => {
                ...
                let child = derive.derivePublic(parent.publicKey, parent.chainCode, index);
