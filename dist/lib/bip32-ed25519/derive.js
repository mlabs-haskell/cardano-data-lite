...
import { extendedToPubkey } from ".//index.js";
export function isIndexHardened(index) {
    return (index & 0x8000_0000) != 0;
}
export function harden(index) {
    return index | 0x8000_0000;
}
export function derivePrivate(extendedKeyParent, chainCodeParent, index) {
    let Z, chainCode;
    ...
    let kL = bigIntToBytesLE((bytesToBigIntLE(ZL) << 3n) + bytesToBigIntLE(kLP), 32);
    let kR = bigIntToBytesLE((bytesToBigIntLE(ZR) + bytesToBigIntLE(kRP)) % 2n ** 256n, 32);
...
export function derivePublic(parentPublicKey, parentChainCode, index) {
    if (isIndexHardened(index))
...
export function bigIntToBytesLE(n, size) {
    if (n < 0n)
        throw new Error("can't serialize negative bigints");
    ...
    if (n != 0n)
        throw new Error("bigint doesn't fit in " + size + " bytes");
    return bytes;
}
function truncateToRight32Bytes(bytes) {
