...
const naclAny = nacl; // workaround: .d.ts doesn't export nacl.lowlevel even though .js does.
const { scalarbase, pack25519, M, gf, S, crypto_hash, modL } = naclAny.lowlevel;
/**
 * Convert extended key to public key.
 */
export function extendedToPubkey(extendedKey) {
 ...
 * the extendedKey.
 */
export function signExtended(msg, extendedKey) {
    if (extendedKey.length !== 64)
        throw new Error("bad extended key size");
 ...
 * of the secret.
 */
function crypto_sign_extended(sm, m, n, xk) {
