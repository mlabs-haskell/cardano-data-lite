import { BigNum, BigInt, } from "../generated.js";
import { add_fractions, ceil_division, div_fractions, fraction_one, fraction_zero, is_fraction_negative_or_zero, mul_fractions, pow_fraction, sub_fractions, toBigIntFloor, } from "./fraction.js";
export class LinearFee {
    _constant;
    _coefficient;
    constructor(coefficient, constant) {
...
export function calculate_exunits_ceil_cost(exUnits, exUnitPrices) {
    const memPrice = exUnitPrices.mem_price();
    const stepsPrice = exUnitPrices.step_price();
    const memRatio = multiply_unit_interval_by_bignum(memPrice, exUnits.mem());
    const stepsRatio = multiply_unit_interval_by_bignum(stepsPrice, exUnits.steps());
    const totalCostFraction = add_fractions(memRatio, stepsRatio);
    return ceil_division(totalCostFraction);
}
export function min_script_fee(tx, exUnitPrices) {
...
function tier_ref_script_fee(multiplier, sizeIncrement, baseFee, totalSize) {
    ...
    if (fullTiers > 0) {
        ...
        const tierProgressionSum = div_fractions(progressionEnumerator, progressionDenominator);
        const tierPriceTimesProgressionSum = mul_fractions(tierPrice, tierProgressionSum);
...
export function min_ref_script_fee(totalRefScriptsSize, refScriptBigNumsPerByte) {
    const multiplier = {
    ...
    return tier_ref_script_fee(multiplier, sizeIncrement, refMultiplier, totalRefScriptsSize);
}
